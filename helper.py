...
class ps():
    ...
    def search(begin, end, precision):
        ...
        while abs(begin - end) > 1e-9:
            mid = (begin + end)/2
            a = (self.f(mid) - self.f(pivot))/(mid-pivot)
            b = self.f(mid) - a*mid
            if self.error(a,b,pivot,mid) <= precision:
                begin = mid
            else:
                end = mid
        return (begin + end)/2
    ...
    def approximate(precision,eps):
        ...
        curr = 2
        while curr > eps:
            next = self.search(curr,0,precision)
            a = np.array([    [next  , 1],
                              [curr, 1]   ])
            b = np.array([    f(next),
                              f(curr)        ])
    ...
    def represent(precision,eps):
        lines = self.approximate(precision,eps)
        a = lines[:,0]
        c = lines[:,1]
